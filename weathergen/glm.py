...
class LinearEffects(AbstractEffects):
    ...
    def __init__(
        ...
        coef = numpyro.sample(name, dist.MultivariateNormal(self.prior_loc, self.prior_cov))
...
class InteractionEffects(AbstractEffects):
    ...
    def get_predictors(self, xy: Tuple):
        assert (
            isinstance(xy, Tuple) and len(xy) == 2
        ), "input to get_predictors for interaction effect must be a tuple of length 2"
        x, y = xy
