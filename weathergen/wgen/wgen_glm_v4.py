...
def wgen_glm_v4(
    ...
    **kwargs,
    ...
    # precipitation
    precip_step = wgen_glm_v4_precip(
    ...
    def step(
        state, inputs, obs={"prec": None, "Tavg": None, "Trange": None, "Tskew": None}
    ):
        assert (
        ...
        Tavg, Tavg_mean, Tavg_mean_seasonal = tair_mean_step(
        ...
        Trange, Tskew, Tmin, Tmax = tair_range_skew_step(
            (Tavg, prec), inputs, obs["Trange"], obs["Tskew"]
        )
...
def wgen_glm_v4_Tair_mean(
    ...
    **kwargs,
    ...
    Tavg_lag_effects = numpyro.sample(
        "Tavg_lag", dist.MultivariateNormal(jnp.zeros(order), 0.2 * jnp.eye(order))
    )
    ...
    Tavg_anom_effects = jnp.concat(
        [Tavg_lag_effects, Tavg_seasonal_lag1_effects, Tavg_pred_effects], axis=-1
    )
    ## residual scale
    ...
    log_Tavg_scale_all_effects = jnp.concat(
        [log_Tavg_scale_seasonal_effects, log_Tavg_scale_pred_effects], axis=-1
    )
    ...
    def step(state, inputs, Tavg_obs=None):
        ...
        i, year, month, doy = inputs[:, :4].T
        predictors = inputs[:, 4:]
        ff_t = utils.fourier_feats(i, freqs, intercept=False) * jnp.ones(
            (Tavg_prev.shape[0], 1)
        )
        ...
        Tavg_mean_anom_features = jnp.concat(
            [Tavg_prev, seasonal_lag1, predictors], axis=1
        )
        log_Tavg_scale_features = jnp.concat([ff_t, predictors], axis=1)
        Tavg_mean = numpyro.deterministic(
            "Tavg_mean",
            Tavg_mean_seasonal
            + jnp.sum(Tavg_mean_anom_features * Tavg_anom_effects, axis=1),
        )
        Tavg_scale = numpyro.deterministic(
            "Tavg_scale",
            jnp.exp(
                jnp.sum(log_Tavg_scale_features * log_Tavg_scale_all_effects, axis=1)
            ),
        )
        ...
        with mask(mask=Tavg_mask):
            if Tavg_dof_mean is not None:
                ...
                Tavg = numpyro.sample(
                    "Tavg", dist.Normal(Tavg_mean, Tavg_scale), obs=Tavg_obs
                )
...
def wgen_glm_v4_precip(
    ...
    **kwargs,
    ...
    precip_occ_Tavg_effects = numpyro.sample(
        "precip_occ_Tavg", dist.MultivariateNormal(jnp.zeros(1), 0.1 * jnp.eye(1))
    )
    ...
    def step(state, inputs, prec_obs=None):
        ...
        prec_occ_features = jnp.concat(
            [ff_t, 1 - prev_wet, log_prev_prec, Tavg_anom, predictors], axis=1
        )
        prec_mean_features = jnp.concat(
            [ff_t, 1 - prev_wet, log_prev_prec, Tavg_anom, predictors], axis=1
        )
        prec_shape_features = jnp.concat(
            [ff_t, 1 - prev_wet, log_prev_prec, predictors], axis=1
        )
        # linear predictors
        ...
        eta_precip_shape = jnp.sum(
            prec_shape_features * precip_shape_all_effects, axis=1
        )
        ...
        precip_gamma_shape = numpyro.deterministic(
            "precip_gamma_shape", jnp.exp(eta_precip_shape)
        )
        precip_gamma_rate = numpyro.deterministic(
            "precip_gamma_rate", precip_gamma_shape / jnp.exp(eta_precip_mean)
        )
...
def wgen_glm_v4_Tair_range_skew(
    ...
    **kwargs,
    ...
    ## range mean
    seasonal_dims = 2 * len(freqs)
    Trange_mean_seasonal_effects = numpyro.sample(
        "Trange_mean_seasonal",
        dist.MultivariateNormal(jnp.zeros(seasonal_dims), jnp.eye(seasonal_dims)),
    )
    Trange_mean_Tavg_effects = numpyro.sample(
        "Trange_mean_Tavg", dist.MultivariateNormal(jnp.zeros(1), 0.1 * jnp.eye(1))
    )
    Trange_mean_wet_effects = numpyro.sample(
        "Trange_mean_wet", dist.MultivariateNormal(jnp.zeros(1), jnp.eye(1))
    )
    Trange_mean_pred_effects = numpyro.sample(
        "Trange_mean_pred",
        dist.MultivariateNormal(jnp.zeros(num_predictors), jnp.diag(pred_effect_scale)),
    )
    Trange_mean_all_effects = jnp.concat(
        [
            Trange_mean_seasonal_effects,
            Trange_mean_Tavg_effects,
            Trange_mean_wet_effects,
    ...
    Trange_shape_Tavg_effects = numpyro.sample(
        "Trange_shape_Tavg", dist.MultivariateNormal(jnp.zeros(1), 0.1 * jnp.eye(1))
    )
    Trange_shape_wet_effects = numpyro.sample(
        "Trange_shape_wet", dist.MultivariateNormal(jnp.zeros(1), jnp.eye(1))
    )
    ...
    Trange_shape_all_effects = jnp.concat(
        [
            ...
            Trange_shape_Tavg_effects,
            Trange_shape_wet_effects,
    ...
    Tskew_Tavg_effects = numpyro.sample(
        "Tskew_Tavg", dist.MultivariateNormal(jnp.zeros(1), 0.1 * jnp.eye(1))
    )
    Tskew_wet_effects = numpyro.sample(
        "Tskew_wet", dist.MultivariateNormal(jnp.zeros(1), jnp.eye(1))
    )
    ...
    def step(state, inputs, Trange_obs=None, Tskew_obs=None):
        ...
        Trange_features = jnp.concat(
            [ff_t, jnp.abs(Tavg.reshape((-1, 1))), is_wet, predictors], axis=1
        )
        Tskew_features = jnp.concat(
            [ff_t, Tavg.reshape((-1, 1)), is_wet, predictors], axis=1
        )
        ...
        with mask(mask=Trange_mask):
            Trange = numpyro.sample(
                "Trange",
                dist.Gamma(jnp.maximum(Trange_gamma_shape, 1e-6), Trange_rate),
                obs=Trange_obs,
            )
        ...
        with mask(mask=Tskew_mask):
            Tskew = numpyro.sample(
                "Tskew",
                dist.Beta(
                    Tskew_mean * Tskew_dispersion, (1 - Tskew_mean) * Tskew_dispersion
                ),
                obs=Tskew_obs,
            )
